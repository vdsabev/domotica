module.exports = {
  list: function (req, client, next) {
    ...
    db.Controller.find(query, select, options, function (error, controllers) {
      if (req.join) {
        _.each(controllers, function (controller) {
          client.join('controller:' + controller._id);
        });
      }

      return next(error, controllers);
    });
  },
  view: function (req, client, next) {
    ...
    db.Controller.findById(req.data._id, select, function (error, controller) {
      ...
      if (req.join) {
        client.join('controller:' + req.data._id, { replace: true });
      }
